function SCENE1(){
    ...
    function INIT(){
        ...
        layerForegroundID = SIB_ScreenManager.createScreen(screenWidth,screenHeight);
        SIB_ScreenManager.assignScreenToDivElement(layerForegroundID,'screen');
        layerBackgroundID = SIB_ScreenManager.createScreen(screenWidth,screenHeight);
        SIB_ScreenManager.assignScreenToDivElement(layerBackgroundID,'screen');

        SIB_ScreenManager.setScreenBackgroundColor(layerForegroundID,'transparent');
        ...
        SIB_SpriteManager.loadSpriteSheet('FlickerSpritesheet','art/flickerspritesheet.png');

        birdSpriteID = SIB_SpriteManager.createSprite('Spritesheet');
        birdFrameID = SIB_SpriteManager.selectSprite(birdSpriteID).addFrame([{"x":76,"y":171,"w":32,"h":28},{"x":129,"y":210,"w":32,"h":28}]);
        
        tileGroundSpriteID = SIB_SpriteManager.createSprite('Spritesheet');
        tileGroundFrameID = SIB_SpriteManager.selectSprite(tileGroundSpriteID).addFrame([{"x":186,"y":212,"w":61,"h":64}]); 
        
        mountainSpriteID = SIB_SpriteManager.createSprite('Spritesheet');
        mountainFrameID = SIB_SpriteManager.selectSprite(mountainSpriteID).addFrame([{"x":0,"y":288,"w":640,"h":182}]);

        cactusSpriteID = SIB_SpriteManager.createSprite('Spritesheet');
        cactusFrameID = SIB_SpriteManager.selectSprite(cactusSpriteID).addFrame([{"x":0,"y":105,"w":75,"h":104}]);

        bushSpriteID = SIB_SpriteManager.createSprite('Spritesheet');
        bushFrameID = SIB_SpriteManager.selectSprite(bushSpriteID).addFrame([{"x":0,"y":210,"w":128,"h":77}]);
        ...
        sunFrameID = SIB_SpriteManager.selectSprite(sunSpriteID).addFrame([{"x":279,"y":129,"w":128,"h":128}]);
        
        // SIB_SpriteManager.selectSprite('sun').addFrame('sunBlink',[{"x":279,"y":0,"w":128,"h":128}]);

        screenFlickerSpriteID = SIB_SpriteManager.createSprite('FlickerSpritesheet');
        screenFlickerFrameID = SIB_SpriteManager.selectSprite(screenFlickerSpriteID).addFrame([
            {"x":0,"y":0,"w":569,"h":320},
        ...
        SIB_SpriteManager.registerGameObjectForSpriteRequest(birdID,birdSpriteID,true);
        ...
        SIB_SpriteManager.registerGameObjectForSpriteRequest(tileGroundID,tileGroundSpriteID,false);
        SIB_SpriteManager.spriteRequest(tileGroundID).setIdleSpriteFrame(tileGroundFrameID,1);

        mountainID = SIB_GameObjectManager.createGameObject();
        SIB_SpriteManager.registerGameObjectForSpriteRequest(mountainID,mountainSpriteID,false);
        SIB_SpriteManager.spriteRequest(mountainID).setIdleSpriteFrame(mountainFrameID,1);
        SIB_GameObjectManager.selectGameObject(mountainID).setPosition(0,75);

        sunID = SIB_GameObjectManager.createGameObject();
        SIB_SpriteManager.registerGameObjectForSpriteRequest(sunID,sunSpriteID,false);
        ...
        SIB_SpriteManager.registerGameObjectForSpriteRequest(cactusID,cactusSpriteID,false);
        SIB_SpriteManager.spriteRequest(cactusID).setIdleSpriteFrame(cactusFrameID,1);
        SIB_GameObjectManager.selectGameObject(cactusID).setPosition(200,152);

        bushID = SIB_GameObjectManager.createGameObject();
        SIB_SpriteManager.registerGameObjectForSpriteRequest(bushID,bushSpriteID,false);
        ...
        SIB_SpriteManager.registerGameObjectForSpriteRequest(coinID,coinSpriteID,true);
        SIB_SpriteManager.spriteRequest(coinID).setIdleSpriteFrame(coinFrameID,1);
        SIB_SpriteManager.spriteRequest(coinID).playSpriteAnimationFrame(coinFrameID,10);
        ...
        SIB_SpriteManager.registerGameObjectForSpriteRequest(screenFlickerID,screenFlickerSpriteID,true);
        SIB_SpriteManager.spriteRequest(screenFlickerID).setIdleSpriteFrame(screenFlickerFrameID,1);
        SIB_SpriteManager.spriteRequest(screenFlickerID).setSpriteAnimationOverlayFrame(screenFlickerFrameID,2);
        SIB_SpriteManager.spriteRequest(screenFlickerID).playSpriteAnimationOverlay(4);

        SIB_GameObjectManager.setGameObjectsLayer([birdID,tileGroundID,screenFlickerID,cactusID,bushID,coinID],layerForegroundID);
        SIB_GameObjectManager.setGameObjectsLayer([sunID,mountainID],layerBackgroundID);
        ...
        tileGroundSpriteRequest.tileSetAmount(Math.ceil(screenWidth/tileGroundSize.w)+1);
        ...
        SIB_CollissionManager.registerGameObjectForCollissionRequest(birdID,true,true,true);
        SIB_CollissionManager.registerGameObjectForCollissionRequest(coinID,false,true,true);
        
        coinTiledPosition = SIB_GameObjectManager.selectGameObject(coinID).getPosition();
...
;
        ...
        if(keyboard.arrowLeft){
            ...
            birdSpriteRequest.playSpriteAnimationFrame(birdFrameID,flapAnimationSpeedHigh);
        ...
        if(!keyboard.spaceBar){
            birdSpriteRequest.playSpriteAnimationFrame(birdFrameID,flapAnimationSpeedMedium);
        }
        ...
        if((bushPosition.x+bushSize.w) < 0)
            bushPosition.x = screenWidth;

        if((cactusPosition.x+cactusSize.w) < 0)
            cactusPosition.x = screenWidth;
        ...
        do{
            ...
            if((tileGroundPosition[i]._position.x+tileGroundSize.w) < 0){
                ...
                i == 0 ? tileGroundPosition[i]._position.x = tileGroundPosition[tileGroundPositionLength-1]._position.x+tileGroundSize.w-groundVelocity
                :tileGroundPosition[i]._position.x = tileGroundPosition[i-1]._position.x+tileGroundSize.w;
            }
            i++;
        }while(--len);
        ...
        do{
            ...
            if((coinTiledPosition[i]._position.x+coinTiledSize.w) < 0){
                ...
                if(coinTiledPosition[i]._hide)
                    coinTiledPosition[i]._hide = false;  
            }
            i++;
        }while(--len);	
    }
